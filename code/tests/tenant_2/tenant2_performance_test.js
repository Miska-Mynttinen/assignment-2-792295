...
async function runPerformanceTest() {
    ...
    await saveSourceDataSize(testData.slice((0.6 * testDataLength), (testDataLength)))
    const iterationTimes = [];
    
    /* Items at the end of the source data */
    for (let i = 6; i < 10; i++) {
        ...
        for (let j = startIndex; j < endIndex; j++) {
            await inputDirectory.putFilesIntoInputDirectory(testData[j], '2');
        }
...
async function saveIterationTimes(iterationTimes) {
    const relativeFilePath = path.join(__dirname, 'tenant_2_performance_test.json');
    await fs.writeFile(relativeFilePath, JSON.stringify(iterationTimes));
}

async function saveSourceDataSize(sourceData) {
    const relativeFilePath = path.join(__dirname, 'tenant_2_performance_source_data_size.json');
